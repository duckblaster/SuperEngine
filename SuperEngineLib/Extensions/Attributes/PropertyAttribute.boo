namespace SuperEngine.Extensions.Attributes
...
class PropertyAttribute(Boo.Lang.Compiler.AbstractAstAttribute):
	...
	override def Apply(node as Node):
		typeDefinition as TypeDefinition = node.GetAncestor[of TypeDefinition]()
		propName = _propertyName.Name
		if _setPreCondition is not null:
			_setPreCondition = [| $node != value and $_setPreCondition |]
		else:
			_setPreCondition = [| $node != value |]
		prop = [|
			public $propName as $_propertyType:
				get:
					return $node
				set:
					if $_setPreCondition:
		...
		|]
		if IsObservable:
			eventArgsName = propName + "ChangedEventArgs"
			eventDefName = propName + "Changed"
			eventHandlerName = "On" + eventDefName
			oldPropName = "Old" + propName
			eventArgs = [|
				public class $eventArgsName(EventArgs):
					$_propertyType oldVal
					$_propertyType newVal
					
					public OldPosition as $_propertyType:
						get:
							return oldVal
					...
					public def constructor(_oldVal as $_propertyType, _newVal as $_propertyType):
						oldVal = _oldVal
						newVal = _newVal
			|]
			eventDef = [|
				public event $eventDefName as EventHandler[of $eventArgs]
			|]
			eventHandler = [|
				protected virtual def $eventHandlerName(e as $eventArgs):
					$eventDef(self, e)
			|]
			prop = [|
				public $propName as $_propertyType:
